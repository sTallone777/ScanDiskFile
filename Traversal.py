...
#登録の目標となるDriverを洗い出す
def GetTargetDriver():
    exceptDriver = appSetting['ExceptDriver']
    # M_DRIVERのDriverを抽出
    cursor.execute('SELECT DRIVER_ID, DRIVER_NAME FROM M_DRIVER')
    result = cursor.fetchall()
    #drivers = [x[0] for x in GetDrivers()]
    targetDriver = [x for x in result if x[1] not in exceptDriver]
    return targetDriver
...
def CreateFileTypeMaster(extension):
    cursor.execute('SELECT TYPE_ID FROM M_FILETYPE WHERE TYPE_NAME = %s', [extension])
    createDateTime = updateDateTime = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
    typeCount = cursor.fetchone()

    if typeCount is None:
        cursor.execute('SELECT MAX(0 + RIGHT(TYPE_ID, 5)) FROM M_FILETYPE')
        maxNum = cursor.fetchone()
        ...
        cursor.execute('INSERT INTO M_FILETYPE(TYPE_ID, TYPE_NAME, SORT_KEY, CREATE_NAME, CREATE_DATETIME, '
                       'UPDATE_NAME, UPDATE_DATETIME) VALUES(%s, %s, %s, %s, %s, %s)',
                       [typeStartN + '{:0>5d}'.format(int(maxId)), extension, createName, createDateTime,
                        updateName, updateDateTime])
        log.logger.info(
            'insert data: [{0}], [{1}]'.format(typeStartN + '{:0>5d}'.format(int(maxId)), extension))
        return typeStartN + '{:0>5d}'.format(int(maxId))
    else:
        return typeCount[0]
...
def CleanTable():
    cursor.execute('TRUNCATE TABLE M_FOLDER')
    cursor.execute('TRUNCATE TABLE T_FILEINFO')
...
def dirTraversal(driverId, driverName):
    ...
    for root, dirs, files in os.walk(driverName):
        ...
        for dir in dirs:
            ...
            parentId = 'null'
            if dirDeep > 1:
                #parent = dirPath.split(os.path.sep)[-2]
                parentId = GetFolderId(dirPath.split(os.path.sep)[1:-1], driverId)[-1]
        ...
        for file in files:
            ...
            #ファイル種類マスタへの登録、更新
            typeId = CreateFileTypeMaster(extension)
            #ファイル情報登録・更新
            CreateFileRecord(file, typeId, filePath)
            print(filePath)
            j += 1
